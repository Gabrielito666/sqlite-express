...
describe('declareSQL function', () => {
    ...
    describe('Parameter Format Support', () => {
        test('should work with @ prefixed parameters', async () => {
            const options = new Options([{ route: '/test' }]);
            options.set({
                db: dbType,
            ...
            const fn = await declareSQL(options);
            const result = await fn({ '@name': 'John Doe', '@age': 25 });
            expect(typeof result).toBe('object');
            expect(result).not.toBeNull();
            // @ts-ignore - Test environment, we know the result structure
            expect(result.name).toBe('John Doe');
        ...
        test('should work with normal parameters (without @)', async () => {
            const options = new Options([{ route: '/test' }]);
            options.set({
                db: dbType,
                query: 'SELECT * FROM users WHERE name = @name AND age = @age',
            ...
            const fn = await declareSQL(options);
            const result = await fn({ name: 'Jane Smith', age: 30 });
            expect(typeof result).toBe('object');
            expect(result).not.toBeNull();
            // @ts-ignore - Test environment, we know the result structure
        ...
        test('should work with mixed parameter formats', async () => {
            const options = new Options([{ route: '/test' }]);
            options.set({
                db: dbType,
                query: 'SELECT * FROM users WHERE name = @name AND age = @age',
            ...
            const fn = await declareSQL(options);
            const result = await fn({ '@name': 'Bob Johnson', age: 35 });
            expect(typeof result).toBe('object');
            expect(result).not.toBeNull();
            // @ts-ignore - Test environment, we know the result structure
        ...
        test('should handle INSERT with normal parameters', async () => {
            const options = new Options([{ route: '/test' }]);
            options.set({
                db: dbType,
                query: 'INSERT INTO users (name, age, city, active) VALUES (@name, @age, @city, @active)',
            ...
            const fn = await declareSQL(options);
            const result = await fn({ name: 'New User', age: 40, city: 'Miami', active: 1 });
            expect(typeof result).toBe('number');
            expect(result).toBeGreaterThan(0);
        });

        test('should handle UPDATE with normal parameters', async () => {
            const options = new Options([{ route: '/test' }]);
            options.set({
                db: dbType,
                query: 'UPDATE users SET age = @newAge WHERE name = @name',
                type: 'update',
                logQuery: false
            });
            const fn = await declareSQL(options);
            const result = await fn({ newAge: 26, name: 'John Doe' });
            expect(typeof result).toBe('number');
            expect(result).toBe(1);
        });

        test('should handle DELETE with normal parameters', async () => {
            const options = new Options([{ route: '/test' }]);
            options.set({
                db: dbType,
                query: 'DELETE FROM users WHERE name = @name',
            ...
            const fn = await declareSQL(options);
            const result = await fn({ name: 'Bob Johnson' });
            expect(typeof result).toBe('number');
            expect(result).toBe(1);
        });
