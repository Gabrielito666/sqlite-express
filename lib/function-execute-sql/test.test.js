...
describe('executeSQL function', () => {
    ...
    describe('Path-based SQL Queries', () => {
        ...
        test('should execute SELECT query from SQL file', async () => {
        ...
        test('should execute INSERT query from SQL file', async () => {
            const sqlContent = 'INSERT INTO users (name, age, city, active, salary) VALUES (@name, @age, @city, @active, @salary)';
            ...
            verifyOptions.set({
                ...
                parameters: { name: 'File User' },
                expected: 'row',
                type: 'select',
                logQuery: false
            });
            const verifyResult = await executeSQL(verifyOptions);
        ...
        test('should execute complex SQL from file with formatting', async () => {
            const sqlContent = `
                SELECT 
                    u.name,
                    u.age,
            ...
            options.set({
                ...
                db: dbType,
                query: sqlFilePath,
                parameters: { active: 1, minSalary: 50000, limit: 3 },
                expected: 'rows',
                type: 'select',
                logQuery: false
            });
        ...
        test('should handle empty SQL file', async () => {
            ...
            // Debe fallar porque el archivo está vacío
            await expect(executeSQL(options)).rejects.toThrow();
        });
    });
});
