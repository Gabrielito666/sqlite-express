...
const getTestPath = () =>
{
	const testPath = path.resolve(__dirname, `testDB-${randomBytes(10).toString("hex")}.db`);
...
describe("DB class tests", () =>
{
	...
	test("declareSQL with and without finalize method", async() =>
	{
		const testPath = getTestPath();
		...
		const stmt1 = await db.declareSQL({ query: "INSERT INTO users(name, age) VALUES(@name, @age)" });
		const stmt2 = await db.declareSQL({ query: "SELECT age FROM users WHERE name = @name" });
		stmt1.insert({ name: "John", age: 27 });

		const ageJohn = await stmt2.select.celd({ name: "John" });

		expect(ageJohn).toBe(27);

		//finalize just one stmt and this not is an error
