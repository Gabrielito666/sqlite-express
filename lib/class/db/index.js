...
const simpleMethodWrap = function(method)
{
    return (args) =>
    {
        ...
        return ScopesQueue.awaitScopeStart(args.scope, scopesQueue).then(() =>
        {
            return method(sqliteDb, {logQuery: this.logQuery, ...args});
        })
    }
};
...
const stmtMethodWrap = function(method, stmtKey)
{
    return (args) =>
    {
        ...
        const scopeStartPromise = ScopesQueue.awaitScopeStart(args.scope, scopesQueue);

        return Promise.all(/**@type {[Promise<any>, Promise<Statement>]}*/([ scopeStartPromise, stmtPromise ]))
        .then(([_, stmt]) =>
        {
            return method(stmt, {logQuery: this.logQuery, ...args});    
        });
    }
}
    ...
    close()
    {
        return new Promise((resolve, reject) =>
        {
            ...
            .then(() => {
                ...
                Promise.all(/**@type {Promise<Statement>[]}*/([
                    beginDefferredTransactionStamentPromise,
                    beginExclusiveTransactionStamentPromise,
                    beginImmediateTransactionStamentPromise
                ]))
