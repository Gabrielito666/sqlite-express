 ...
 * @typedef {import("./types").DBPrivate} DBPrivate
*/
...
const DB = class
{
    ...
    */
    constructor({route, logQuery=true})
    {
        ...
        privateCamps.stmtPromisesList = /**@type {DBPrivate["stmtPromisesList"]}*/([]);
    ...
    */
    createTable(args)
    {
        ...
        return ScopesQueue.Scope
        .getPrivate(scope)
        .addOperation(createTable, [sqliteDb, this], {logQuery : this.logQuery, ...args});
    }
    ...
    */
    declareSQL(args)
    {
        const stmtPromise = simpleMethodWrap.bind(this)(declareSQL)(args);
        DB.getPrivate(this).stmtPromisesList.push(stmtPromise);
        return stmtPromise;
    }
    ...
    */
    close()
    {
        return new Promise((resolve, reject) =>
        {
            ...
            .then(stmtArr => //This is a sqlite3 stmt aray
            {
                ...
                return Promise.allSettled(finalizePromises)
            })
            .then(() => Promise.allSettled(DB.getPrivate(this).stmtPromisesList)) //this is a sqlite-express declareSQL stmt. are diferent sqlite3 stmt
            .then(stmtsArr => Promise.allSettled(stmtsArr.map(stmt => 
            {
                ...
                {
                    console.error("Error in finalize statements.", stmt.reason);
                }
